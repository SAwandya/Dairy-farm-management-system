...
function TemperatureDisplay() {
  ...
  useEffect(() => {
    const socket = new WebSocket('ws://localhost:303'); // Connect to WebSocket server
    ...
    socket.onmessage = (event) => {
      ...
      setExceedsLimit(data.temperature > 39);
    };
  ...
  return (
    <Card sx={{ maxWidth: 345, 
               ...
               // backgroundColor: temperature > 31 ? '#ffcccc' : 'inherit', // Change background color based on temperature
                border: temperature ? (temperature > 36 ? '5px solid red' : '5px solid blue') : '5px solid black',
                backgroundColor: temperature && temperature > 36 ? '#ffcccc' : '#C4E4FF',
    }}>
      <CardContent>
        ...
        >
          {temperature !== null ? (
            ...
            sx={{ color: temperature && temperature > 36 ? 'red' : 'inherit' }}>{temperature !== null ? temperature.toFixed(1) : 'N/A'} °c</Typography>
          ) : (
            <CircularProgress />
          )}
        </div>
