...
function TemperatureDisplay() {
  ...
  const initiatePlayback = () => {
    const audio2 = new Audio(entry);
    audio2.play().then(() => {
    }).catch(error => {
      console.error('Failed to initiate playback:', error);
    });
  };
  // Initiate playback of muted audio when component mounts
  useEffect(() => {
    initiatePlayback();
  }, []);
  ...
  return (
    <Card sx={{ maxWidth: 345, 
      ...
      <CardContent>
        ...
        <Typography align="center" variant="h6" component="h3">Temperature Range: 20-33</Typography>
